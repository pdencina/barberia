        ...
        {clients.length > 0 && (
          <div className="flex gap-2">
              ...
              onClick={async () => {
                ...
                if (data.links) {
                  data.links.forEach((l: any, i: number) => {
                    setTimeout(() => window.open(l.url, "_blank"), i * 1500);
                  });
                  showToast(`Abriendo ${data.links.length} conversaciones WhatsApp`, "success");
                }
              }}
              disabled={bulkSending}
            ...
            >
              ...
              WhatsApp Masivo ({clients.filter((c) => c.phone).length})
            </button>
          </div>
        )}
      </div>
