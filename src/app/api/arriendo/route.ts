...
export async function GET(req: NextRequest) {
  ...
  // Get product sales per barber (for bonuses)
  ...
  const txBarberMap = new Map((transactions || []).map((t) => [t.id, t.barber_id]));
  ...
  const recordMap = new Map((existingRecords || []).map((r) => [r.barber_id, r]));

  // Build summary per professional
  const summary = professionals.map((prof) => {
    const existing = recordMap.get(prof.id);
  ...
  const totals = {
    totalGross: summary.reduce((s, p) => s + p.grossAmount, 0),
    totalNet: summary.reduce((s, p) => s + p.netAmount, 0),
...
export async function PATCH(req: NextRequest) {
  ...
  if (recordId) {
    ...
    const { data: prof } = await supabase.from("profiles").select("rental_daily_rate").eq("id", barberId).single();
    const dailyRate = Number(prof?.rental_daily_rate) || 29000;
